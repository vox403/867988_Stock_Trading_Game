...
async function buyStock(symbol) {
  ...
  try {
    ...
    if (message.includes("DAILY_INVEST_USED")) {
      ...
      setLog("이미 보유 중인 주식이 있습니다. 매도 후 다음 오전 9시 갱신을 기다리십시오.");
...
function fillInvest(symbol, type) {
  ...
  writeInvestAmount(symbol, Math.min(Math.max(1, amount), state.cashWon));
}
...
async function boot() {
  ...
  try {
    await refreshAll();
    setLog("시장 세션 확인 완료. 등락률 데이터가 활성화되었습니다.");
  } catch (error) {
