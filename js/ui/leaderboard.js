...
export function renderLeaderboard(rows, playerKey) {
  ...
  if (selfRow) {
    const name = selfRow.display_name || selfRow.displayName || "UNKNOWN";
    els.selfRank.textContent = `${rankValue(selfRow)}위 · ${name} · ${holdingLabel(selfRow)}`;
  } else {
    els.selfRank.textContent = "순위 산출 대기";
  }
}
