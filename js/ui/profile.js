...
export function renderProfile(profile) {
  ...
  els.coinBalance.textContent = formatCoins(profile.balance);
  els.totalEquity.textContent = formatCoins(profile.totalEquity);
  ...
  if (profile.positionSymbol) {
    ...
    els.positionText.textContent = `${profile.shares.toFixed(6)}주 · 평균 ${formatCoins(profile.avgPrice)} 코인 · 현재 평가 ${formatCoins(profile.positionValue)} 코인`;
  ...
  els.positionText.textContent = profile.canInvest ? "투자할 회사의 투자 버튼을 눌러 코인을 입력하십시오." : "다음 오전 9시 갱신 이후 다시 투자할 수 있습니다.";
