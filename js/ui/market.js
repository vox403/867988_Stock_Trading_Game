...
function createStockCard(row, canInvest) {
  ...
  const openButton = actionButton("stock-order-open", "투자", { orderOpen: symbol });
  openButton.disabled = !canInvest;
  if (!canInvest) openButton.dataset.blocked = "true";
...
export function renderMarket(rows, canInvest) {
  ...
  els.marketList.replaceChildren(...list.map((row) => createStockCard(row, canInvest)));
}
